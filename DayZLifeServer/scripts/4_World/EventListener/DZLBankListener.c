    ...
    void HandleEventsDZL(PlayerIdentity sender, Object target, int rpc_type, ParamsReadContext ctx) {
        if (rpc_type == DAY_Z_LIFE_PLAYER_DEPOSIT_AT_BANK_DATA) {
            ...
            if (ctx.Read(paramDeposit)){
                ...
                GetGame().RPCSingleParam(paramDeposit.param1, DAY_Z_LIFE_PLAYER_DEPOSIT_AT_BANK_DATA_RESPONSE, new Param1<string>(message), true);
            }
        } else if (rpc_type == DAY_Z_LIFE_PLAYER_BANK_DATA) {
            autoptr Param2<PlayerBase, float> paramGetBankData;
            if (ctx.Read(paramGetBankData)){
                GetGame().RPCSingleParam(paramGetBankData.param1, DAY_Z_LIFE_PLAYER_BANK_DATA_RESPONSE, new Param1<ref DZLBank>(bank), true, sender);
            }
        }
    }
}