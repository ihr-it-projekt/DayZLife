        ...
        if(building) {
            ...
            if(doorIndex != -1) {
                ...
                if(GetGame().IsServer()) {
                    ...
                    if((DZLConfig.Get().adminIds.HasAccess(DAY_Z_LIFE_JOB_COP, dzlPlayer.dayZPlayerId) && dzlPlayer.IsActiveJob(DAY_Z_LIFE_JOB_COP)) || (DZLConfig.Get().adminIds.HasAccess(DAY_Z_LIFE_JOB_ARMY, dzlPlayer.dayZPlayerId) && dzlPlayer.IsActiveJob(DAY_Z_LIFE_JOB_ARMY))) {
                        return true;
                    }
