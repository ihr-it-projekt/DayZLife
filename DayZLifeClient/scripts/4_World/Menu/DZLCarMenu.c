class DZLCarMenu : DZLBaseMenu {
    ...
    override void OnShow() {
        ...
        g_Game.RPCSingleParam(car, DZL_RPC.GET_CAR_KEYS, null, true);
        g_Game.RPCSingleParam(null, DZL_RPC.GET_DAY_Z_LIFE_ALL_PLAYER_ONLINE_PLAYERS_FOR_ALL, null, true);
    }
    ...
    override bool OnClick(Widget w, int x, int y, int button) {
        ...
        if(w == carKeySaveButton) {
            g_Game.RPCSingleParam(player, DZL_RPC.GET_UPDATE_CAR_KEYS, new Param2<CarScript, ref array<DZLOnlinePlayer>>(car, DZLDisplayHelper.GetPlayerIdsAndRanksFromList(carPanelKeyOwnerList)), true);
            ...
            g_Game.RPCSingleParam(player, DZL_RPC.CHANGE_CAR_OWNER, new Param2<string, CarScript>(selectedPlayer.id, car), true);
            OnHide();
        }
    ...
    override void HandleEventsDZL(PlayerIdentity sender, Object target, int rpc_type, ParamsReadContext ctx) {
        if(rpc_type == DZL_RPC.GET_CAR_KEYS_RESPONSE) {
            Param1<ref array<ref DZLOnlinePlayer>> paramOnlinePlayers;
            ...
            Param1<ref array<ref DZLOnlinePlayer>> paramAllPlayers;
